...
1.  **建立檔案副本**：
    *   請將客製化選項選擇登記萬用系統試算表 (檔案名稱可以任意修改) 複製到您自己的 Google 雲端硬碟。
...
2.  **設定 Google 試算表**：
    ...
    *   **「名單」工作表**：準備使用者名單。至少需要「帳號」欄位 (通常為電子郵件地址)。根據您的需求，可以加入「密碼」(若啟用帳密登入)、「姓名」、「編組1」、「編組2」、「編組3」、「選項」等欄位。
    *   **「編組」工作表**：若您需要使用督導郵件功能，請在此工作表設定各編組的督導人員及其電子郵件。（第2層管理人員，幫您分組督導各組的每一位用戶是否完成登記或填報）
