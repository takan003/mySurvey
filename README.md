...
2.  **設定 Google 試算表**：
    ...
    *   **「編組」工作表**：若您需要督導人員協助分組管理，請在此工作表設定各編組的督導人員及其電子郵件。（第2層管理人員，幫您分組督導各組的每一位用戶是否完成登記或填報）
...
1.  請從 [[客製化選項選擇登記萬用系統](https://example.com/your-project-folder-link](https://drive.google.com/drive/folders/1MiXMgUm1YfLw19XUWLXt5bUcpNBP3cHm?usp=drive_link)] 複製整個專案資料夾到您的 Google 雲端硬碟。
    *   資料夾內應包含：
...
1.  **「設定」工作表**：
    ...
    *   **用詞與稱呼**:
        ...
        *   `用戶稱呼`: 例如：學員、成員、用戶 (將影響提示文字及郵件內容)。
    *   **編組設定**:
        *   `編組1代名詞`: 例如：班級、部門 (例如在網頁顯示班級而不是編組1)。
        *   `編組2代名詞`: 例如：組別、科系 (在網頁顯示的替代用詞)。
        *   `編組3代名詞`: 例如：學號、職稱 (在網頁顯示的替代用詞)。
    *   **選項設定**:
        *   `選項名稱`: 整個選項區塊的標題，例如：選修課程、報名項目。
        *   `選項內容`: 用戶可勾選的項目，請用**英文逗號** (`,`) 分隔，例如：選項A,選項B,選項C。
        *   `選項屬性`: 填寫 `單選` 或 `多選`。
        *   `必須做出選擇`: 填寫 `啟用` 時，用戶在封存前必須至少選擇一個項目 (若為多選) 或做出選擇 (若為單選)。留空或填寫其他則不強制。
    *   **郵件與通知**:
